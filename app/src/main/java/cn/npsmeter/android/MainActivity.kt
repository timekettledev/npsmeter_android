                    ...
                    { type ->
                        when (type) {
                            ...
                            NPSCloseType.MinFatigue -> {
                                val ts = Toast.makeText(baseContext, "距离上次弹出问卷时间过短", Toast.LENGTH_SHORT)
                                ts.show()
                            }
                            NPSCloseType.FirstDay -> {
                                val ts = Toast.makeText(baseContext, "距离首次下载配置时间过短", Toast.LENGTH_SHORT)
                                ts.show()
                            }
                            NPSCloseType.HaveShowForId -> {
                                val ts = Toast.makeText(baseContext, "已经显示过该问卷", Toast.LENGTH_SHORT)
                                ts.show()
                            }
                            NPSCloseType.RequestAnswerError -> {
                                val ts = Toast.makeText(baseContext, "回答问卷请求出错", Toast.LENGTH_SHORT)
                                ts.show()
                            }
